...
from minepy import MINE # for calculating the MIC
import seaborn as sb
from scipy.stats import pearsonr # for calculating Pearson correlation
from scipy.interpolate import interp1d, lagrange # for interpolating the original data

# Set figure size based on x and y length
xlength = 3.27*2.2
...
# updating custom configuration settings
...
def multi_plot():
    ...
    filepath = "./pollutants_Filter.xlsx"
    df = pd.read_excel(filepath, sheet_name='S8', nrows=8036)
    tempt = df['tempt']
    CO = df['CO']
    NO = df['NO']
    NO2 = df['NO2']
...
def mic_pearson():
    ...
    filepath = "./pollutants_Filter.xlsx"
    df = pd.read_excel(filepath, sheet_name='S8', nrows=8036,usecols=np.arange(1,9,1).tolist())
